      ...
      <div class="col-xs-6 col-sm-4">
        ...
        <a href="<?= $href; ?>" title="<?= $row['pname']; ?>">
          <div class="product-image" style="background-image:url(<?= $row['filename']; ?>)">
              <?php if($row['bProductDiscount']) { ?>  <span class="extra_discount_small">Extra Discount!</span> <?php } ?>
          </div>
        </a>
      </div>
      <div class="col-xs-6 col-sm-8">
        ...
        <div class="widget-price-container">
          ...
          <?php if(isset($row['saleprice']) && ($row['price'] != $row['saleprice'])) { ?>
            <div class="col-md-12 text-left">
              <div class="row">
                <small class="text-sale price">Sale: <span class="amount_wd"><?= $row['format_sale_price']; ?></span></small>
              </div>
            </div>
          <?php } ?>
        </div>
