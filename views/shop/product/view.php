              ...
              <div class="row">
                <div class="row">
                  ...
                  <div class="btn-group product-detail-view-actions" role="group" aria-label="...">
                    <?php $pid = _A_::$app->get('pid'); if ($data['inventory'] > 0) { ?>
                      <a id="add_cart"
                         href="<?= _A_::$app->router()->UrlTo('cart/add', ['pid' => $pid]) ?>" <?= !isset($in_cart) ? 'class="button visible-item"' : 'style="display: none;" class="button"'; ?>>
                        Add to cart
                      </a>
                      <a id="view_cart"
                         href="<?= _A_::$app->router()->UrlTo('cart') ?>" <?= isset($in_cart) ? 'class="button visible-item"' : 'style="display: none;" class="button"'; ?>>
                        Basket
                      </a>
                    <?php } ?>
                    <?php if ($data['inventory'] > 0 && $allowed_samples) { ?>
                      <a id="add_samples_cart" href="<?= _A_::$app->router()->UrlTo('cart/add_samples', ['pid' => $pid]) ?>" <?= !isset($in_samples_cart) ? 'class="button visible-item"' : 'style="display: none;" class="button"'; ?>>
                        Add Samples
                      </a>
                    <?php } ?>
                    <?php
                    ...
                    ?>
                    <a class="button visible-item" href="<?= $ahref; ?>">Ask a Question</a>
                    <?php if ($img1_exists) { ?>
                      <a id="add_matches"
                         href="<?= $mhref; ?>" <?= !isset($in_matches) ? 'class="button visible-item"' : 'style="display: none;" class="button"'; ?>>
                        Added to Matches
                      </a>
                      <a id="view_matches"
                         href="<?= _A_::$app->router()->UrlTo('matches'); ?>" <?= isset($in_matches) ? 'class="button visible-item"' : 'style="display: none;" class="button"'; ?>>
                        View Matches
                      </a>
                    <?php } ?>
                  </div>
