...
<?php
    if ($prev_page > 0) {
        $opt['page'] = $prev_page;
        echo '<a class="prev page-numbers" href="' . _A_::$app->router()->UrlTo('admin_home', $opt) . '" >Prev</a>';
...
<?php
    if ($page > 1) {
        $opt['page'] = 1;
        echo '<a class="prev page-numbers" href="' . _A_::$app->router()->UrlTo('admin_home', $opt) . '" >First</a>';
...
for ($i = $nav_start; $i <= $nav_end; $i++) {
    if ($i == $page) {
        ...
        ?>
        <li>
            <a class="prev page-numbers" href="<?= _A_::$app->router()->UrlTo('admin_home', $opt); ?>"><?= $i ?></a>
        </li>
        <?php
    }
}
...
<li>
    ...
    if ($page < $last_page) {
        $opt['page'] = $last_page;
        echo '<a class="prev page-numbers" href="' . _A_::$app->router()->UrlTo('admin_home', $opt) . '" >Last</a>';
...
<li>
    ...
    if ($next_page <= $last_page) {
        $opt['page'] = $next_page;
        echo '<a class="prev page-numbers" href="' . _A_::$app->router()->UrlTo('admin_home', $opt) . '" >Next</a>';
