<script type="text/javascript">
    function sendComment(address){
        ...
        jQuery.post(address, {comment_data : data, comment_title : title}, function(data){
            if(data.length > 0){
                    jQuery("#comment-message-error").html(data);
                    if(jQuery("#comment-message-error-success").length > 0){
                        jQuery("#comment-form-head").hide(400);
                        jQuery("#comment-form-data").hide(400);
                        jQuery("#comment-form-save").hide(400);
                    }
                }
            });
        }
