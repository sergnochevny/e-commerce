...
<tr>
  <td><span class="cut-text-in-one-line"><b><?= $row['discount_amount']; ?>
        % off </b><?= $row['discount_comment1']; ?></span></td>
  <td>
  ...
  <?php if(!isset($hide_action)) { ?>
    <td>
      <div class="text-center">
        <a data-waitloader rel="nofollow" href="<?= _A_::$app->router()->UrlTo('discount/edit', $opt); ?>">
          <i class="fa fa-pencil"></i>
        </a>
        <a data-del_discount rel="nofollow" href="<?= _A_::$app->router()->UrlTo('discount/del', $opt); ?>">
          <i class=" fa fa-trash-o"></i>
        </a>
        <a data-waitloader class="text-success" rel="nofollow"
           href="<?= _A_::$app->router()->UrlTo('discount/usage', $opt); ?>">
