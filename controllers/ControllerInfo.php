...
class ControllerInfo extends ControllerFormSimple{
  ...
  protected function before_form_layout(&$data = null){
    if(!empty($data)) {
      $this->main->view->setVars('form_title', $this->title_scenario[$this->scenario()]);
  ...
  protected function form($url, $data = null, $return = false){
    ...
    if($return) return $this->RenderLayoutReturn('form', $return && App::$app->RequestIsAjax());

    return $this->RenderLayout('form');
  }
  ...
  public function view($partial = false, $required_access = false, $as_widget = false){
    if($as_widget || App::$app->RequestIsAjax()) {
      if(!is_null($this->scenario()) && in_array($this->scenario(), array_keys($this->resolved_scenario))) {
        ...
        $this->main->view->setVars('data', $data);
        if($as_widget) return $this->RenderLayoutReturn('view/' . $this->scenario());
        $this->RenderLayout('view/' . $this->scenario(), !$as_widget);
