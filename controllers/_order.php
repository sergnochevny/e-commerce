...
//        } else {
        ...
        if (!empty($_GET['order_id'])) {
            $back_url .= '?order_id=' . $_GET['order_id'];
        }
...
//            $back_url = $_SESSION['last_url'];
//        } else {
        $back_url = BASE_URL . (empty($_GET['discounts_id']{0}) ? '/discounts' : '/usage_discounts');

        if (!empty($_GET['discounts_id']{0})) {
            $back_url .= '?discounts_id=' . $_GET['discounts_id'];
        }
//        }
        $this->get_order_details();
        $userInfo = $model->get_order($order_id);
    ...
    {
        $user_id = (integer)$_SESSION['user']['aid'];
        $page = (integer)(empty($_GET['page']) ? 0 : $_GET['page']);
    ...
    public function orders_history()
    {
        ...
        if (isset($_GET['orders_search_query'])) {
            $like = htmlspecialchars(trim($_GET['orders_search_query']));
            $page = (integer)(empty($_GET['page']) ? 0 : $_GET['page']);
            $this->template->vars('page', $page);
    ...
    public function order_info()
    {
        ...
        $oid = (integer)urldecode(base64_decode(isset($_GET['oid']) ? $_GET['oid'] : null));
        $back_url = BASE_URL . '/orders_history' . '?page=' . $page;
        if (isset($_GET['orders_search_query'])) {
            $back_url = $back_url . '&orders_search_query=' . $_GET['orders_search_query'];
        }
