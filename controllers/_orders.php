  ...
  class Controller_Orders extends Controller_Controller {
    ...
    private function get() {
      ...
      if(!empty($orders_count) && ((int)$orders_count > 0)) {
        $rows = Model_Order::get_orders_by_user($user_id, $start, $limit, $res_count_rows);
    ...
    public function customer_history() {
      ...
      if($page > ceil($total_pages / $per_page))
        $page = ceil($total_pages / $per_page);
      if($page <= 0)
        $page = 1;
      $start = (($page - 1) * $per_page);
      $config = (array)['aid' => $user_id, 'from' => $start, 'to' => $per_page,];
    ...
    public function history() {
      ...
      if(is_null(_A_::$app->get('orders_search_query'))) {
        ...
        $config = ['like' => $like, 'from' => $start, 'to' => $per_page];
        ...
        if(count($orders) > 0) {
          ...
          foreach($orders as $order) {
            ...
            $handling = strlen(trim($handling)) > 0 ? '$' . number_format($handling, 2) : '';
            $end_date = $end_date ? date('F m, Y', strtotime($end_date)) : '';
            $action = _A_::$app->router()->UrlTo('orders/info', ['oid' => $oid, 'page' => $page, 'orders_search_query' => _A_::$app->get('orders_search_query')]);
            $this->template->vars('action', $action);
        ...
        if(count($orders) > 0) {
          ...
          foreach($orders as $order) {
            ...
            $end_date = $end_date ? date('F m, Y', strtotime($end_date)) : '';
            $action = _A_::$app->router()->UrlTo('orders/info', ['oid' => urlencode(base64_encode($oid))]);
            $this->template->vars('action', $action);
            $this->template->vars('date', $date);
            $this->template->vars('end_date', $end_date);
            $this->template->vars('track_code', $track_code);
