...
class ModelPrice extends ModelBase{
   ...
   * @param $bCodeValid
   * @param $bReturnString
   * @param $sPriceDiscount
   * @param $sShippingDiscount
   * @param $shipType
   * @param $discountIds
   * @return float|int
   * @throws \Exception
          ...
          if($bReturnString && $rs['discount_type'] == DISCOUNT_TYPE_SHIPPING) {
            ...
            if($bReturnString) {
              ...
              if($rs['discount_type'] == DISCOUNT_TYPE_SUBTOTAL) {
                if($isDiscountApplay) $temp = sprintf("reduced further by %s.", $tempAmtString); else $temp = sprintf("Reduced by %s.", $tempAmtString);
...
//        if($rs = static::FetchAssoc($result)) {
  ...
  public static function calculateProductSalePrice($pid, $price, &$discountIds){
    ...
    #grab the user id
    $uid = (isset(App::$app->session('user')['aid'])) ? (int)App::$app->session('user')['aid'] : 0;
